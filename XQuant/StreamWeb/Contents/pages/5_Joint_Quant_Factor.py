...
from XQuant import IMPLEMENTED, JointQuant, Analyzer, Strategy, RtnResult, format_date
import pandas as pd
...
def get_df(joint_quant, factor_name):
    st.session_state.barra_factor_df = getattr(joint_quant, factor_name)
...
def JointQuantFactor():
    ...
    all_data = ChainMap(IMPLEMENTED.raw, IMPLEMENTED.joint_quant)
    ...
    joint_quant_factor = JointQuant(
        begin=begin, end=end, bench_code=bench_code, parallel=False
    )
    ...
    if col1.button("获取/显示数据", key="get_barra_button", use_container_width=True):
        joint_quant_factor.begin = begin
        joint_quant_factor.end = end
        with st.spinner("请等待"):
            st.session_state.barra_factor_df = getattr(
                joint_quant_factor, all_data[data_name]
            )
    ...
    if col3.button("ICIR", key="cal_ICIR_button", use_container_width=True):
        with st.spinner("请等待"):
            ...
            if len(st.session_state.barra_factor_df > 0) and data_name != "bench":
                returns = joint_quant_factor.returns
